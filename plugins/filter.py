...
async def auto_filter(client, message):
    ...
    try: await client.send_message(Config.LOG_CHANNEL, f"🔍 **Search:** `{message.text}`\n👤: {message.from_user.mention}")
    except: pass

    files = await db.search_files(message.text)
    if not files:
        try: return await status.edit("❌ **No results found.**")
        except: return
    ...
    try: await status.edit(f"✅ **Found {len(files)} results:**", reply_markup=InlineKeyboardMarkup(btn))
    except: pass
